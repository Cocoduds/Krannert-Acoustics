...
void TimerHandler0(void) {
  ...
  while( ADC0->STATUS.bit.ADCBUSY == 1 ); 
    ...
    // fram.write8(sam_curr, ((uint16_t)val >> 0) & 0xFF);
    // fram.write8(sam_curr+1, ((uint16_t)val >> 8) & 0xFF);
    // sam_curr+=2;
    ...
    if (samp_full == 1) {
      ...
      if(myFile){
        for (int j = 0; j < DATA_SIZE; j+=1){
          // myFile.println((fram.read8(j) << 4) | fram.read8(j+1));
          myFile.println(aud_sampl[j]);
        }
      } else {
